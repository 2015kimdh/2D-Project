...
class Enemy_3:
    ...
    def __init__(self, x, y, xpull, ypull, dx, dy):
        ...
        if Enemy_3.image == None:
            Enemy_3.image = load_image('enemy.png')
        if Enemy_3.sound == None:
            Enemy_3.sound = load_wav('Bomb.wav')
            Enemy_3.sound.set_volume(20)
        self.time = 0
    ...
    def update(self):
        if self.time < self.slice:
            if self.move == 0:
                ...
                if self.y < 600 and self.y > 250:
                    ...
                    self.randomnumy = 1
            if self.move == 1:
                self.x += FLY_SPEED_PPS/8 * game_framework.frame_time * self.randomnumx
                self.y += FLY_SPEED_PPS/8 * game_framework.frame_time * self.randomnumy
        ...
        if self.Hp > 400:
            self.bullet_phase0()
        elif self.Hp < 400 and self.Hp > 200:
            self.bullet_phase1()
        elif self.Hp < 200:
            self.bullet_phase2()
